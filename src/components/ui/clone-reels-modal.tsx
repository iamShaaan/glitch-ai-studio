...
import { CLONE_USE_CASE_REELS } from "@/lib/video-config";
import { claimAudioFocus, subscribeToAudioClaims } from "@/lib/video-sync";

interface CloneReelsModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export function CloneReelsModal({ isOpen, onClose }: CloneReelsModalProps) {
  ...
  const total = CLONE_USE_CASE_REELS.length;
  ...
  useEffect(() => {
    ...
    CLONE_USE_CASE_REELS.forEach((reel, idx) => {
  ...
  const toggleMute = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (!nextMuted) {
        claimAudioFocus(3000 + currentIndex, video);
        if (video.paused) {
          video.play().catch(() => {});
          setIsPaused(false);
        }
      ...
      >
        ...
        >
          ...
          <div className="relative w-full h-full flex items-center justify-center">
            {CLONE_USE_CASE_REELS.map((reel, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Clean Bottom Overlay: Title of use case */}
                  <div
        ...
        <div className="relative z-30 flex items-center justify-center gap-1.5 py-1">
          {CLONE_USE_CASE_REELS.map((_, idx) => (
            <button
              key={idx}
              onClick={() => {
                setCurrentIndex(idx);
