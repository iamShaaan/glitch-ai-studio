...
import { TRAVEL_INFLUENCER_REELS } from "@/lib/video-config";
import { claimAudioFocus, subscribeToAudioClaims } from "@/lib/video-sync";

interface TravelReelsModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export function TravelReelsModal({ isOpen, onClose }: TravelReelsModalProps) {
  ...
  const total = TRAVEL_INFLUENCER_REELS.length;
  ...
  useEffect(() => {
    ...
    TRAVEL_INFLUENCER_REELS.forEach((reel, idx) => {
  ...
  const toggleMute = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (!nextMuted) {
        claimAudioFocus(2000 + currentIndex, video);
        if (video.paused) {
          video.play().catch(() => {});
          setIsPaused(false);
        }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 md:p-6 animate-in fade-in duration-200">
      ...
      {/* Floating Window Container - Complete 100% focus on video */}
      <div
        ...
        onWheel={handleWheel}
      >
        {/* Subtle Close Button in Top-Right (No headers, no titles, no badges) */}
        ...
        >
          ...
          <div className="relative w-full h-full flex items-center justify-center">
            {TRAVEL_INFLUENCER_REELS.map((reel, idx) => {
              ...
              // Mobile ~185px, Tablet ~240px, Desktop ~295px
              const baseSpacing =
                windowWidth < 640 ? 185 : windowWidth < 1024 ? 240 : 295;

              const translateX = diff * baseSpacing + dragOffset;

              // Smooth intensity scaling based on distance from middle:
              // Middle (0): blur(0px), opacity: 1, scale: 1
              // Side (±1): smooth semi-blur (blur 3px), opacity: 0.72, scale: 0.88
              // Outer (±2): medium blur (blur 8px), opacity: 0.42, scale: 0.76
              // Far (±3): hard blur (blur 14px), opacity: 0.18, scale: 0.66
              // >3: hidden
              let blurPx = 0;
              let opacity = 1;
              let scale = 1;
              ...
              return (
                ...
                >
                  ...
                  {/* Clean Bottom Overlay: ONLY the visiting place or activity name */}
                  <div
                    className={`absolute bottom-0 inset-x-0 p-4 pt-12 bg-gradient-to-t from-black/90 via-black/40 to-transparent pointer-events-none flex flex-col justify-end transition-opacity duration-300 ${
                      isMiddle ? "opacity-100" : "opacity-0"
                    }`}
                  >
                    <h3 className="font-anton text-xl sm:text-2xl md:text-3xl uppercase text-white tracking-[0.035em] drop-shadow-md text-center leading-tight">
                      {reel.title}
                    </h3>
                  </div>
                </div>
        ...
        {/* Minimal Bottom Wheel Indicator Dots (No text, pure visual) */}
        <div className="relative z-30 flex items-center justify-center gap-1.5 py-1">
          {TRAVEL_INFLUENCER_REELS.map((_, idx) => (
            <button
              key={idx}
              onClick={() => {
                setCurrentIndex(idx);
